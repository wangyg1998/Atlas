...
{
	data = cholmod_l_zeros(m, n, CHOLMOD_REAL, Common::Common());
}
...
{
	return DenseMatrix(cholmod_l_eye(m, n, CHOLMOD_REAL, Common::Common()));
}

inline DenseMatrix DenseMatrix::ones(size_t m, size_t n)
{
	return DenseMatrix(cholmod_l_ones(m, n, CHOLMOD_REAL, Common::Common()));
}
...
{
	return cholmod_l_norm_dense(data, norm, Common::Common());
}
...
{
	return cholmod_l_copy_dense(data, Common::Common());
}
...
{
	cholmod_l_free_dense(&data, Common::Common());
	data = NULL;
}
