...
{
	if (factor) {
		cholmod_l_free_factor(&factor, Common::Common());
		factor = NULL;
	}
...
inline void Cholesky::buildSymbolic(cholmod_sparse *C)
{
	...
	factor = cholmod_l_analyze(C, Common::Common());
	if (factor) validSymbolic = true;
}

inline void Cholesky::buildNumeric(cholmod_sparse *C)
{
	if (factor) validNumeric = (bool)cholmod_l_factorize(C, factor, Common::Common());
}
...
inline bool Cholesky::solvePositiveDefinite(DenseMatrix& x, DenseMatrix& b)
{
	...
	if (factor)
	{
		x = cholmod_l_solve(CHOLMOD_A, factor, b.toCholmod(), common);
	}
